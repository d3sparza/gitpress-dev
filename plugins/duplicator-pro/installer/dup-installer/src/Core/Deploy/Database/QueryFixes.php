...
class QueryFixes extends AbstractJsonSerializable
{
     ...
     *
     * @deprecated 4.0.6.2
     * @deprecated This feature is very old and should fix the spaces maybe when migration come from Windows. It is believed to be no longer necessary
     *
     * @return void
     *
     */
    protected function rulesNbspFix()
    {
        if (!PrmMng::getInstance()->getValue(PrmMng::PARAM_DB_SPACING)) {
            return;
        }

        $this->globalRules['search'][] = '/\xC2\xA0/';
        // replace 2 chars with 2 chars to avoid object serialized corruption
        $this->globalRules['replace'][] = '  ';
        $this->generatorLog .= "GLOBAL RULES ADDED: NBSP\n";
    }
